	...
	count($sections) == 0)
	{
      ...
      <tr>
		...
		foreach ($sections as $num => $section) {
			...
			echo '<td>';
			echo '&nbsp;&nbsp;';
			if($section['type']=='anim' || $section['type']=='tree' || $section['type']=='deco' || $section['type']=='building' || $section['type']=='seed') echo '<input onclick="self.location.href=\'main.php?vEditPlace='.$num.'&vPlace='.($section['place']<>0?'0':'1').'\'"  type="checkbox" name="place" value="1" ' . ($section['place']==1 ? 'checked ' : ''). '>';
			echo '</td>';
